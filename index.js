    ...
    for (let i = 1; i < objects.length; i++) {
      for (let name in objects[i]) {
        if (objects[i].hasOwnProperty(name)) {
          if (!firstObject.hasOwnProperty(name) || Object.prototype.toString.call(
              objects[i][name]) !== '[object Object]') {
...
const formatting = (date, string, options) => {
  if (!date && Object.prototype.toString.call(date) !== '[object Date]') {
    return undefined;
  }

  options = (options && Object.prototype.toString.call(options) === '[object Object]')
    ? assignDeep({}, defaults, options)
    : defaults;

  string = (!string || typeof string !== 'string')
    ? `${options.format.monthShort} ${options.format.day}, ${options.format.yearFull} ${options.format.hour}:${options.format.minutes} ${options.format.timezone}`
    : string;

  // Date parse
  let dateParse = {};
  dateParse[options.format.deyWeekMin] = options.daysShortNames[date.getDay()];
  ...
  dateParse[options.format.monthShort] = options.monthShortNames[date.getMonth()];
  dateParse[options.format.monthFull] = options.monthFullNames[date.getMonth()];
  dateParse[options.format.yearShort] = date.getFullYear().toString().slice(-2);
  ...
  dateParse[options.format.hourAmPmMin] = date.getHours() > 12 ? (date.getHours() - 12).toString() : date.getHours().toString();
  dateParse[options.format.hourAmPm] = date.getHours() > 12 ? ('0' + (date.getHours() - 12)).slice(-2) : ('0' + date.getHours()).slice(-2);
  ...
  dateParse[options.format.milliseconds] = ('00' + date.getMilliseconds()).slice(-3);
  ...
  for (let s of string) {
    if (stringArr.length && s !== ' ' && formatStr.match(
        new RegExp(`\\s(${stringArr[stringArr.length - 1] + s})[\\S]*\\s`, 'g'))) {
      stringArr[stringArr.length - 1] = stringArr[stringArr.length - 1] + s;
    }
    else {
      stringArr.push(s);
    }
